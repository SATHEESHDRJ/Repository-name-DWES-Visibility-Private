...
- Initial bootstrap: self-signed at `${DATA_ROOT}/ssl/nginx/fullchain.pem` + `privkey.pem`
  (the path nginx actually mounts — `docker-compose.yml` `${DATA_ROOT}/ssl/nginx:/etc/nginx/ssl:ro`)
