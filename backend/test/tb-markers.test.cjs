...
/** Stub ManualEndpointMappingService that always returns empty list. */
function makeManualMappingSvc() {
  return { list: () => [] };
}
...
test('TB marker matching: header-group paint; prefer exact terminal when present', async () => {
  ...
  const matchSvc = new TbMarkerMatchService(prisma, undefined, makeManualMappingSvc());
  const supervisor = makeUser({ id: 10, role: 'prod_supervisor' });
  ...
  assert.equal(res1.best_source?.geometry?.paint_mode, 'header_group', 'always header_group, never terminal_cell');
