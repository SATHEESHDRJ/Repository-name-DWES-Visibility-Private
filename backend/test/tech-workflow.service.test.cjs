...
test('TechService.changeover carries wiring progress and links immutable history', async () => {
  ...
  const closed = updates.find((u) => u?.data?.changeover_locked === true)
    || updates.find((u) => u?.data?.handover_to_id != null)
    || updates[updates.length - 1];
  assert.ok(closed, 'expected an update that closes the prior assignment');
  assert.equal(closed.data.handover_to_id, 2);
  assert.equal(closed.data.changeover_locked, true);
