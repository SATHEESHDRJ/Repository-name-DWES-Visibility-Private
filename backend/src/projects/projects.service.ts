...
import { MockStore, FrameData, User } from '../data/mock-store';
...
import { permanentlyDeleteProject, type PermanentDeleteActor } from '../common/project-delete.util';
...
import { qaQcWorkflowEnabled } from '../common/feature-flags';
import {
  buildDirectorSubmissionAuditDetails,
  countOpenEnds,
  directorSubmissionKey,
  isPanelWiringCompleteForDirector,
} from '../common/director-submission.util';
...
export class ProjectsService {
  ...
  async remove(code: string, actor?: PermanentDeleteActor) {
    ...
    return permanentlyDeleteProject(this.prisma, code, uploadBase, actor);
  }
  ...
  /**
   * Supervisor-gated publish of a completed panel to the Operations Director.
   * Per-panel idempotency: project_code + frame_id + action submitted_to_director.
   * QA/QC gates apply only when QA_QC_WORKFLOW_ENABLED=true.
   */
  async submitToDirector(
    code: string,
    actor?: Pick<User, 'id' | 'full_name' | 'username'>,
    target?: { frameId?: string; assignmentId?: number },
  ) {
    const p = await this.prisma.projects.findFirst({ where: { code, is_active: true } });
    if (!p) throw new NotFoundException(`Project ${code} not found`);

    const qaEnabled = qaQcWorkflowEnabled();
    const actorId = actor?.id ?? 0;
    const actorName = (actor?.full_name || actor?.username || 'Supervisor').trim();

    if (qaEnabled && !target?.frameId && !target?.assignmentId) {
      return this.submitProjectToDirectorLegacy(code, p, actorId, actorName);
    }

    const frameId = target?.frameId?.trim();
    const assignmentId = target?.assignmentId;
    if (!frameId && !assignmentId) {
      throw new BadRequestException('Panel frame or assignment id is required');
    }

    let assignment = await this.resolveDirectorSubmitAssignment(code, frameId, assignmentId);
    if (!assignment) throw new NotFoundException('Panel assignment not found');

    const subKey = directorSubmissionKey(code, assignment.frame_id);
    const existingAudit = await this.prisma.tech_audit_log.findFirst({
      where: {
        project_code: code,
    ...
    if (existingAudit) {
      return {
        message: 'Panel already submitted to director',
        project_code: code,
        frame_id: assignment.frame_id,
        assignment_id: assignment.id,
        submission_status: 'submitted',
        submitted_by: existingAudit.technician_name || actorName,
        submitted_at: existingAudit.created_at?.toISOString() || null,
        idempotent: true,
      };
    }

    if (!isPanelWiringCompleteForDirector(assignment)) {
      throw new BadRequestException('Panel wiring is not complete — cannot submit to director');
    }

    if (qaEnabled) {
      if (assignment.review_status === 'ready_for_qc') {
        throw new BadRequestException('Panel is still pending QC check');
      }
      if (assignment.review_status === 'rework') {
        throw new BadRequestException('Panel is still marked for rework');
      }
    }

    const tech = await this.prisma.users.findUnique({ where: { id: assignment.technician_id } });
    const technicianName = tech?.full_name || tech?.username || `Technician #${assignment.technician_id}`;
    const cs = parseCS(assignment.cable_status);
    const cablesTotal = assignment.cables_total ?? 0;
    const cablesCompleted = Object.values(cs).filter(state => Boolean(state?.src && state?.dst)).length;
    const { openEndSource, openEndDestination } = countOpenEnds(assignment.cable_status);
    const submittedAt = new Date();
    const reportRef = `DWES-PCR-${`${code}-${assignment.panel_name || assignment.frame_id}`.toUpperCase().replace(/[^A-Z0-9-]/g, '').slice(0, 30)}`;

    const details = buildDirectorSubmissionAuditDetails({
      projectCode: code,
      frameId: assignment.frame_id,
      panelName: assignment.panel_name || assignment.frame_id,
    ...
    await this.prisma.tech_audit_log.create({
      data: {
        technician_id: actorId || assignment.technician_id,
    ...
    return {
      message: 'Panel submitted to director',
      project_code: code,
      frame_id: assignment.frame_id,
      assignment_id: assignment.id,
      submission_status: 'submitted',
  ...
  /** Legacy whole-project submission when QA/QC workflow is enabled. */
  private async submitProjectToDirectorLegacy(
    code: string,
    p: { code: string; project_state: string | null },
    actorId: number,
    actorName: string,
  ) {
    if (p.project_state === 'submitted_to_director') {
      throw new BadRequestException('Project is already submitted to director');
    }

    const completed = await this.prisma.tech_assignments.findMany({
      where: { project_code: code, status: 'completed', is_hidden: false },
    });
    ...
    const reworkPanels = completed.filter(a => a.review_status === 'rework').length;
    if (reworkPanels > 0) {
      throw new BadRequestException(`${reworkPanels} panel(s) still marked for rework`);
    }

    const submittedAt = new Date();
    const needsStamp = completed.filter(a => a.review_status !== 'approved');
    for (const a of needsStamp) {
      await this.prisma.tech_assignments.update({
        where: { id: a.id },
        data: {
          review_status: 'approved',
          reviewed_by: actorId || a.reviewed_by,
          reviewed_at: submittedAt,
          review_notes: a.review_notes || 'Submitted to Director (supervisor confirmation; QA/QC pending workflow)',
        },
      });
    }
    ...
    for (const a of completed) {
      const subKey = directorSubmissionKey(code, a.frame_id);
      const existing = await this.prisma.tech_audit_log.findFirst({
        where: { project_code: code, frame_id: a.frame_id, action: 'submitted_to_director' },
      });
      if (existing) continue;

      const tech = await this.prisma.users.findUnique({ where: { id: a.technician_id } });
      const technicianName = tech?.full_name || tech?.username || `Technician #${a.technician_id}`;
      ...
      const reportRef = `DWES-PCR-${`${code}-${a.panel_name || a.frame_id}`.toUpperCase().replace(/[^A-Z0-9-]/g, '').slice(0, 30)}`;
      const details = buildDirectorSubmissionAuditDetails({
        projectCode: code,
        frameId: a.frame_id,
      ...
      await this.prisma.tech_audit_log.create({
        data: {
          technician_id: actorId || a.technician_id,
          technician_name: actorName,
          project_code: code,
          frame_id: a.frame_id,
    ...
    return {
      message: `Project submitted to director with ${completed.length} completed panel(s)`,
      project_code: code,
      submission_status: 'submitted',
      submitted_by: actorName,
      submitted_at: submittedAt.toISOString(),
      panel_count: completed.length,
      panels: completed.map(a => ({
        frame_id: a.frame_id,
        panel_name: a.panel_name || a.frame_id,
        assignment_id: a.id,
      })),
  ...
  /** Latest wiring assignment for director submit (includes hidden technician-dashboard rows). */
  private async resolveDirectorSubmitAssignment(
    projectCode: string,
    frameId?: string,
    assignmentId?: number,
  ) {
    if (assignmentId) {
      const byId = await this.prisma.tech_assignments.findFirst({
        where: { id: assignmentId, project_code: projectCode },
      });
      if (byId) return byId;
    }
    ...
    if (!fid) return null;
    const rows = await this.prisma.tech_assignments.findMany({
      where: { project_code: projectCode, frame_id: fid },
      orderBy: { id: 'desc' },
      take: 1,
    });
    return rows[0] ?? null;
