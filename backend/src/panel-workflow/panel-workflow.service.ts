...
export class PanelWorkflowService {
  ...
  async getByPanel(project_code: string, frame_id: string) {
    ...
    if (!row) return null;
    return this.decorateWithWiringAssignment(row);
  }
  ...
  async ensureWorkflow(
    ...
    const decorated = await this.decorateWithWiringAssignment(created);
    return { ...decorated, ensured: true as const };
  }

  /**
   * Heal WIRING stage assignees from active tech_assignments when empty/mismatched,
   * and attach panel_wiring_assignment projection for UI.
   */
  private async decorateWithWiringAssignment<T extends {
    project_code: string;
    frame_id: string;
    stages?: Array<{
      id: number;
    ...
    const techAssignments = this.prisma?.tech_assignments;
    if (!techAssignments?.findFirst) {
      return {
        ...workflow,
        panel_wiring_assignment: null,
        wiring_stage_synced: false,
      } as T & { panel_wiring_assignment: null; wiring_stage_synced: boolean };
    }

    const active = await techAssignments.findFirst({
      where: {
    ...
    const wiringStage = stages.find((s) => String(s.stage_key || '').toUpperCase() === 'WIRING');
    let syncedWithWiringStage = false;

    if (active && wiringStage && this.techService) {
      const assignees = Array.isArray(wiringStage.assignees) ? wiringStage.assignees : [];
      const hasExactMatch =
        assignees.length === 1 && assignees[0].user_id === active.technician_id;
      const mismatched =
        assignees.length === 0
        || assignees.some((a) => a.user_id !== active.technician_id)
        || !assignees.some((a) => a.user_id === active.technician_id);

      if (mismatched && !hasExactMatch) {
        await this.techService.syncWiringStageAssignee(
          workflow.project_code,
          workflow.frame_id,
          active.technician_id,
          active.assigned_by || active.technician_id,
        );
        const refreshed = await this.prisma.panel_workflow_stage_assignees.findMany({
          where: { stage_id: wiringStage.id },
        });
        (wiringStage as { assignees?: unknown }).assignees = refreshed;
        syncedWithWiringStage =
          refreshed.length === 1 && refreshed[0].user_id === active.technician_id;
      } else {
        syncedWithWiringStage = hasExactMatch;
      }
    }
    ...
    if (active) {
      const tech = await this.prisma.users.findUnique({
        where: { id: active.technician_id },
        select: { full_name: true, username: true },
      });
      technicianName = tech?.full_name || tech?.username || undefined;
    }

    const policy = buildAssignmentActionPolicy(active);

    return {
      ...workflow,
      panel_wiring_assignment: active
        ? {
            ...
            can_reassign: policy.can_reassign,
            can_mid_change: policy.can_mid_change,
