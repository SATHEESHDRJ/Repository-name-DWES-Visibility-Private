...
export class EventsController {
  ...
  /** Resolves what this connection is allowed to observe (re-read per event for technicians). */
  private audienceFor(user: User): () => Promise<EventAudience> {
    ...
    return async () => {
      ...
      const rows = await this.prisma.tech_assignments.findMany({
        ...
        select: { frame_id: true, project_code: true },
      });
      const activeCodes = rows.length
        ? (await this.prisma.projects.findMany({
            where: { code: { in: [...new Set(rows.map(r => r.project_code))] }, is_active: true },
            select: { code: true },
          })).map(p => p.code)
        : [];
      const activeSet = new Set(activeCodes);
      const frameIds = rows
        .filter(row => activeSet.has(row.project_code))
        .map(row => row.frame_id);
      return { role, assignedFrameIds: new Set(frameIds) };
    };
  }
}
