...
export interface DwesServerEvent {
  ...
  projectCode?: string;
  frameId?: string;
  /** Tech assignment id when the mutation was assignment-scoped (crimping / wiring). */
  assignmentId?: number;
  /** Cable index within the assignment schedule (0-based). */
  cableIndex?: number;
  /** Permanent wire_id for row-level report refresh. */
  wireId?: string;
  /** FINISHED advance: wire just completed (same as wireId when present). */
  previousWireId?: string;
  /** FINISHED advance: next active permanent wire_id for LIVE ENDPOINT rematch. */
  nextWireId?: string;
  /** FINISHED advance: next cable index (0-based), when known. */
