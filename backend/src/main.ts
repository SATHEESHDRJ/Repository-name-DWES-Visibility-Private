...
async function bootstrap() {
  ...
  const app = await NestFactory.create<NestFastifyApplication>(
    ...
    new FastifyAdapter({
      ...
      // Nest 11 + Fastify 5: @nestjs/platform-fastify no longer depends on
      // @fastify/middie. DWES still registers no Express-style Nest middleware.
      // skipMiddie retained only if the adapter option remains; otherwise ignored.
      skipMiddie: true,
    } as ConstructorParameters<typeof FastifyAdapter>[0]),
