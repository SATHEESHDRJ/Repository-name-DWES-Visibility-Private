...
export class DrawingTbAnalysisService implements OnModuleInit {
  ...
  private async processInline(input: {
    ...
    const emptyFinds =
      headersFound.length === 0 && reviewCandidates.length === 0;
    const noteBlob = (result.notes || []).map(n => String(n)).join('|');
    const ocrFailedOrUnavailable =
      ...
      || (
        ...
        && emptyFinds
        && (workerHealth.ocr_python === 'missing'
          || workerHealth.ocr_python === 'error'
          || workerHealth.ocr_python === 'unconfigured')
      );
    /** Honest schedule↔drawing miss only when extraction completed (searchable or OCR produced text). */
    ...
    const technical =
      ...
      && (
        ocrFailedOrUnavailable
        || (result.notes || []).some(n =>
          /drawing_missing|ocr_stage_failed|pdfjs_unavailable|worker|timeout|ECONNREFUSED/i.test(String(n)),
        )
      );

    // Prefer TECHNICAL_FAILURE over SCHEDULE_DRAWING_MISMATCH when OCR/worker never
    // produced usable text — empty finds are not proof the schedule TB is absent.
    if (technical) {
      status = 'TECHNICAL_FAILURE';
      failedStage =
        ocrFailedOrUnavailable || (workerHealth.ocr_python !== 'ok' && readiness.searchable === false)
          ? 'ocr'
    ...
    } else if (scheduleDrawingMismatch) {
  ...
  async getPanelStatus(projectCode: string, frameId: string) {
    ...
    // Heal stuck ANALYSING rows so the Technician banner cannot stay forever.
    if (derived.stale_analysis && run?.id) {
      const reason =
        'TECHNICAL_FAILURE: TB-location analysis did not complete (stale ANALYSING). Re-run analysis or verify OCR/worker health.';
      try {
        await this.prisma.drawing_tb_analysis_runs.update({
          where: { id: run.id },
          data: {
