...
export function derivePanelLiveTbStatus(input: {
  ...
  /** When set, ANALYSING older than this many ms is treated as TECHNICAL_FAILURE. */
  updatedAt?: Date | string | null;
  staleAfterMs?: number;
}): { panel_status: LiveTbPanelStatus; failed_stage: LiveTbFailedStage; stale_analysis?: boolean } {
  ...
  const staleAfter = Number.isFinite(input.staleAfterMs) && (input.staleAfterMs as number) > 0
    ? (input.staleAfterMs as number)
    : 10 * 60 * 1000;
  let staleAnalysis = false;
  if (inProgress && input.updatedAt) {
    const t = new Date(input.updatedAt).getTime();
    if (Number.isFinite(t) && Date.now() - t > staleAfter) {
      staleAnalysis = true;
    }
  }

  if (inProgress && !staleAnalysis) {
