...
export class TbMarkersController {
  constructor(
    ...
    private readonly manualMappingSvc: ManualEndpointMappingService,
  ) {}
  ...
  /**
   * 2E: Supervisor-facing failed-match diagnostics.
   * Returns normalized endpoint JSON + rejected candidate reasons for authorized roles.
   */
  @Get('match/diagnostics')
  @Roles(...WRITE_ROLES)
  async matchDiagnostics(
    @Query('project_code') project_code: string,
    @Query('frame_id') frame_id: string,
    ...
    if (!project_code || !frame_id) throw new BadRequestException('project_code and frame_id are required.');
    const result = await this.matchSvc.matchWireEndpoints(
      project_code,
      frame_id,
      { device: String(source_device || '').trim(), terminal: String(source_terminal || '').trim() },
      { device: String(dest_device || '').trim(), terminal: String(dest_terminal || '').trim() },
      user,
      drawing_checksum || null,
    );
    return {
      source_endpoint_kind: (result as any).source_endpoint_kind,
      destination_endpoint_kind: (result as any).destination_endpoint_kind,
      ...
      source_match_reason: result.source_match_reason,
      destination_match_reason: result.destination_match_reason,
      source_candidate_count: result.source_candidate_count,
      destination_candidate_count: result.destination_candidate_count,
    };
  }
  ...
  /* ── 2D: Manual Endpoint Mapping (Supervisor/Engineering) ─────────────── */
  ...
  listManualMappings(
    ...
    if (!project_code || !frame_id) throw new BadRequestException('project_code and frame_id are required.');
    return this.manualMappingSvc.list(project_code, frame_id);
  }

  @Post('manual-mappings')
  @Roles(...WRITE_ROLES)
  createManualMapping(
    @Body() body: any,
    @CurrentUser() user: User,
  ) {
    if (!body?.project_code || !body?.frame_id) {
      throw new BadRequestException('project_code and frame_id are required.');
    }
    return this.manualMappingSvc.create(body, user.id, user.role);
  }
}
