...
# Keep stdout JSON-only; route library noise to stderr.
warnings.filterwarnings("ignore")
os.environ.setdefault("TESSDATA_PREFIX", os.environ.get("TESSDATA_PREFIX", "/usr/share/tesseract-ocr/5/tessdata"))
try:
...
ELIGIBLE = {"REAR_WIRING_VIEW", "INTERNAL_VIEW", "PHYSICAL_TB_BANK", "PHYSICAL_DEVICE"}
REJECTED = {
...
def is_legend_or_directory_context(*text_parts: str) -> bool:
    """True when OCR neighbourhood is a legend/BOM/directory table, not a physical strip."""
    blob = re.sub(r"\s+", "", " ".join(str(t or "") for t in text_parts).upper())
    if not blob:
        return False
    if re.search(r"(LEGEND:?REFDESCRIPTION|BILLOFMATERIALS|DEVICEREF|COMPONENTLIST|PARTSLIST)", blob):
        return True
    if re.search(r"WEIDMULLERW(DU|TL)", blob) and re.search(r"TERMINALFOR", blob):
        return True
    if re.search(r"AUXILIARYRELAY", blob) and re.search(r"FINDER/", blob):
        return True
    return False
...
def ocr_pages_with_boxes(path: str, dpi: int, enrichment: bool) -> list[dict[str, Any]]:
    ...
    lower = path.lower()
    # Soft caps — never mutate original PDF; downscale pixmap for OCR only.
    MAX_EDGE_PX = 8192
    PREVIEW_MAX_EDGE = 4096
    zoom = dpi / 72.0
    angles = [0, 90, 180, 270] if enrichment else [0, 90, 180, 270]

    def process_base(img: Image.Image, page_number: int, notes=None) -> dict[str, Any]:
    ...
    for i, page in enumerate(doc):
        ...
        # Estimate full-res size before allocating a huge pixmap.
        rect = page.rect
        est_w = abs(rect.width) * zoom
        est_h = abs(rect.height) * zoom
        edge = max(est_w, est_h)
        use_zoom = zoom
        if edge > MAX_EDGE_PX or (est_w * est_h) > 40_000_000:
            # Produce optimised OCR preview; preserve original PDF on disk.
            scale = PREVIEW_MAX_EDGE / max(edge, 1.0)
            use_zoom = zoom * scale
            render_notes.append(
                f"oversized_pixmap_downscale;est={int(est_w)}x{int(est_h)};zoom={use_zoom:.4f}"
            )
        mat = fitz.Matrix(use_zoom, use_zoom)
        pix = page.get_pixmap(matrix=mat, alpha=False)
        img = Image.open(io.BytesIO(pix.tobytes("png")))
        pages.append(process_base(img, i + 1, render_notes))
    doc.close()
    return pages
...
def analyse(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for header in expected:
        ...
        physical_pairs: list[tuple[dict[str, Any], dict[str, Any]]] = []
        for page, hit in strip_pairs:
            geom = hit.get("geometry") or {}
            if is_legend_or_directory_context(
                page.get("text") or "",
                geom.get("nearby_compact") or "",
                geom.get("page_compact") or "",
            ):
                notes.append(f"legend_directory_context:{header}:page={page.get('page')}")
                continue
            physical_pairs.append((page, hit))
        if not physical_pairs:
            legend_only.append(header)
            headers_missing.append(header)
            kinds = sorted({h["kind"] for _, h in page_for_hit})
            notes.append(f"non_physical_only:{header}:{','.join(kinds) or 'legend_context'}")
            continue

        # Prefer eligible-view strip hits
        eligible_pairs = [(p, h) for p, h in physical_pairs if h.get("view") in ELIGIBLE]
        use_pairs = eligible_pairs or physical_pairs
...
def main() -> None:
    ...
    # Keep stdout JSON-only: pymupdf/tesseract emit "warning: ..." on stdout.
