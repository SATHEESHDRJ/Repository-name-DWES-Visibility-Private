...
test('PWA manifest keeps the DWES standalone identity and valid icons', () => {
  ...
  assert.equal(
    manifest.name,
    'DWES — Digital Wiring Execution System | Ingenious Network FZC',
  );
...
test('service worker uses network-first HTML and caches hashed build assets', () => {
  const worker = read('public/sw.js');
  assert.match(worker, /url\.pathname\.startsWith\('\/api\/'\)/);
  assert.match(worker, /cache: 'no-store'/);
  assert.match(worker, /20260726-tech-schedule-matrix/);
  assert.match(worker, /isHashedBuildAsset/);
  assert.match(worker, /caches\.open\(STATIC_CACHE\)/);
  assert.match(worker, /key\.startsWith\('dwes-'\)/);
});
...
test('frontend registers the root-scoped worker without HTTP-caching its script', () => {
  ...
  assert.match(html, /<title>DWES — Digital Wiring Execution System \| Ingenious Network FZC<\/title>/);
  ...
  assert.match(registration, /dwes-sw-reloaded/);
