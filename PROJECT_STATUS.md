...
**Last updated:** 2026-07-15 (Release-readiness: `sales_demo` demo account (role `sales_director`, aggregate-only — 9 personnel/audit endpoints 403, `technicianName` redacted, leak-scan clean); `demo_midchg_tech` documented as retained deactivated test history (login 401, excluded everywhere); assignFrame rejects inactive techs; release-guard tests 95/95; popup matrix 9 popups × 6 tablet viewports = 0 defects; prod bundle scan clean. Earlier — Device Preview devtool: `/__device-preview` renders the real app in an exact-viewport iframe (17 tablet presets + custom, rotate/refresh/open-full, DEMO_MODE-gated real-RBAC role switch via `/auth/dev/demo-login`), floating launcher, fully excluded from prod bundles; responsive sweep 6 roles × 20 sizes = 0 overflow/0 small targets. Mid Change relocated into the Pause popup (dashboard button removed) and E2E-verified 59/59 with isolated DEMO data (teardown clean; leftover: deactivated `demo_midchg_tech` account — session-log rows block hard delete). Earlier same day — Session verification pass: typecheck/lint/FE+BE builds green, 84/84 backend tests, health+RBAC for all 5 roles, headless sweep of every role page at desktop+tablet — no console errors/overflow/broken icons; Assign Technician roster fully verified incl. amber ASSIGNED-disabled rows; one CSS fix (`.tech-select-option:disabled` dimming). Note: `=H001` now carries a live assignment from today's testing; `director1` demo account is deactivated — use `ops_director1`. Previous — Assign Technician popup: technician picker is now a custom in-modal listbox — `TechnicianSelect.tsx`, opens downward inside the dialog with internal scroll, name/@username/status-dot rows, assigned techs visible-but-disabled; commit `eaf9023`, headless-verified. Previous same day — Popup modernization: consistent modern leading icons + compact density across all ~35 popups and data-entry fields — shared `Modal` title-icon chip (`icon`/`iconTone`), reusable `.field-with-icon` shell, tone-based confirm/delete chips via `AppDialogProvider`/`DeleteConfirmModal`; `tsc`+`vite build` green, headless-verified. See CHANGELOG. Previous: Complete project-data reset: all 5 canonical seed projects permanently deleted via the app's reset flow; demo project seeding removed from startup **and** dev hard-reset, `seed-projects.ts` deleted; permanent project delete converted from tombstone to true hard delete (numbering reusable after deletion). 39 users, auth, settings, schema, and Fastify migration untouched. Verified: empty supervisor selectors, create+delete lifecycle, restart without reseed. Backups in `backend/uploads/backups/PROJECT_RESET_2026-07-15_*` + `RESET_ALL_2026-07-15T05-15-31*`. **Open bug:** wiring parser drops the first data row — see CHANGELOG)  
