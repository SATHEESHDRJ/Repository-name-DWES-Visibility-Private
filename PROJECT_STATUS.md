...
**Last updated:** 2026-07-15 (Popup modernization: consistent modern leading icons + compact density across all ~35 popups and data-entry fields — shared `Modal` title-icon chip (`icon`/`iconTone`), reusable `.field-with-icon` shell, tone-based confirm/delete chips via `AppDialogProvider`/`DeleteConfirmModal`; `tsc`+`vite build` green, headless-verified. See CHANGELOG. Previous: Complete project-data reset: all 5 canonical seed projects permanently deleted via the app's reset flow; demo project seeding removed from startup **and** dev hard-reset, `seed-projects.ts` deleted; permanent project delete converted from tombstone to true hard delete (numbering reusable after deletion). 39 users, auth, settings, schema, and Fastify migration untouched. Verified: empty supervisor selectors, create+delete lifecycle, restart without reseed. Backups in `backend/uploads/backups/PROJECT_RESET_2026-07-15_*` + `RESET_ALL_2026-07-15T05-15-31*`. **Open bug:** wiring parser drops the first data row — see CHANGELOG)  
