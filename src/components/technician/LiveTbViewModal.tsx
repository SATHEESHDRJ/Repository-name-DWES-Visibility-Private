...
import type { ActiveWireSnapshot, EndpointKind } from '../../types/liveTbView';
import { endpointKindBadgeLabel } from './wiring/wiring-utils';
...
/**
 * Read-only LIVE ENDPOINT VIEW. Wiring mode: Source red / Destination blue.
...
export default function LiveTbViewModal({ open, snapshot, onClose }: Props) {
  ...
  /** True after status API settled (success or failure) — prevents forever "in progress". */
  const [analysisStatusResolved, setAnalysisStatusResolved] = useState(false);
  ...
  const resetState = useCallback(() => {
    ...
    setAnalysisStatusResolved(false);
  ...
  useEffect(() => {
    ...
    // Keep modal mounted across FINISHED auto-advance: update wire endpoints in place.
    // Full freeze only when project/frame changes (or first open).
    setFrozen(prev => {
      if (!snapshot) return null;
      if (
        !prev
        || prev.project_code !== snapshot.project_code
        || prev.frame_id !== snapshot.frame_id
      ) {
        return { ...snapshot };
      }
      if (
        prev.wire_id === snapshot.wire_id
        && prev.cable_index === snapshot.cable_index
        && prev.source_device === snapshot.source_device
        && prev.dest_device === snapshot.dest_device
        && prev.source_terminal === snapshot.source_terminal
        && prev.dest_terminal === snapshot.dest_terminal
      ) {
        return prev;
      }
      // Wire advance / rematch — preserve panel identity so the PDF viewer is not remounted.
    ...
    snapshot?.frame_id,
    snapshot?.wire_id,
    snapshot?.cable_index,
    snapshot?.source_device,
    snapshot?.dest_device,
    snapshot?.source_terminal,
    snapshot?.dest_terminal,
    snapshot?.source_physical_lookup_key,
    snapshot?.dest_physical_lookup_key,
    snapshot?.panel_complete,
  ]); // eslint-disable-line react-hooks/exhaustive-deps -- wire-follow without remount
  ...
  }, [open, frozen?.project_code, frozen?.frame_id, reloadKey, drawingRequests]); // eslint-disable-line react-hooks/exhaustive-deps -- do not reload PDF on wire advance
    ...
    void (async () => {
      try {
        ...
        try {
          ...
          /* Status optional — never leave banner stuck on "in progress". */
        } finally {
          if (matchRequests.isLatest(matchReq.id)) {
            setAnalysisStatusResolved(true);
          }
        }
        ...
        // Header-group match runs for TB and equipment ends. Only skip when there is
        // no schedule identity to look up (legacy empty row).
        const matchSrcDevice = String(frozen.source_device || frozen.source_physical_lookup_key || frozen.source_equipment || '').trim();
        const matchDstDevice = String(frozen.dest_device || frozen.dest_physical_lookup_key || frozen.dest_equipment || '').trim();
        const matchSrcTerm = String(frozen.source_terminal || frozen.source_equipment_terminal || frozen.source_terminal_reference || '').trim();
        const matchDstTerm = String(frozen.dest_terminal || frozen.dest_equipment_terminal || frozen.dest_terminal_reference || '').trim();
        if (!matchSrcDevice && !matchDstDevice) {
          setSourceCandidates([]);
          setDestinationCandidates([]);
          setSamePhysicalGroup(false);
          setSourceResolution(null);
        ...
        const res = await tbMarkersApi.match({
          ...
          source_device: matchSrcDevice,
          source_terminal: matchSrcTerm || (matchSrcDevice ? '—' : ''),
          dest_device: matchDstDevice,
          dest_terminal: matchDstTerm || (matchDstDevice ? '—' : ''),
          drawing_checksum: drawingChecksum,
          signal: matchReq.signal,
        });
        if (!matchRequests.isLatest(matchReq.id)) return;

        // Clear overlays before processing new match results
        setSourceCandidates([]);
        setDestinationCandidates([]);
        setSamePhysicalGroup(false);
        ...
        // Match 1 of N: keep all high-confidence candidates for Previous/Next — never wipe peers.
  ...
  const wireChip = useMemo(() => {
    ...
    const srcKind = (frozen.source_endpoint_kind || null) as EndpointKind | null;
    const dstKind = (frozen.dest_endpoint_kind || null) as EndpointKind | null;
    ...
    return (
      <div className="live-tb-wire-chip" aria-label="Active wire">
        ...
        <span className="live-tb-wire-chip__end live-tb-wire-chip__end--src">
          {srcLabel}
          {srcKind ? <span className="live-tb-wire-chip__badge">{endpointKindBadgeLabel(srcKind)}</span> : null}
        </span>
        <span className="live-tb-wire-chip__end live-tb-wire-chip__end--dst">
          {dstLabel}
          {dstKind ? <span className="live-tb-wire-chip__badge">{endpointKindBadgeLabel(dstKind)}</span> : null}
        </span>
      </div>
    );
  ...
  // Only treat as "still loading status" while match/status fetch is in flight —
  // never after status API has settled with an empty/unknown value.
  const analysisStatusPending = !analysisStatusResolved && matchLoading && !!frozen && !noGa && !drawingError;
  ...
  const analysisFailed = [
    'FAILED', 'TECHNICAL_FAILURE', 'DETECTION_FAILED',
  ].includes(analysisStatus) || panelStatus === 'TECHNICAL_FAILURE';
  ...
  const scheduleMismatch =
    ...
    || mismatchFromSummary;
  /** Technician-facing pipeline chip: QUEUED | PROCESSING | READY | PARTIAL | FAILED */
  const analysisLifecycleChip = (() => {
    ...
    if (analysisReady || activeSource || activeDestination) return 'READY';
  ...
  if (emptyWire) {
    ...
    bannerMessage = drawingError === FORBIDDEN_TECH
      ...
      : drawingError;
  } else if (tbFieldsMissing && !matchLoading && !activeSource && !activeDestination && !markersConfigured) {
    ...
    if (srcDev || dstDev || srcEq || dstEq) {
      bannerMessage = 'Header group not found';
    } else {
    ...
    // Header group not found — from match reason (unique-or-fail or ambiguous)
    const srcReason = String((frozen as any)?.source_match_reason || '');
    const dstReason = String((frozen as any)?.destination_match_reason || '');
    const srcHeaderNotFound = srcReason.startsWith('Header group not found');
    const dstHeaderNotFound = dstReason.startsWith('Header group not found');
    if (srcHeaderNotFound && !activeSource && activeDestination) {
      bannerMessage = 'Header group not found for Source endpoint.';
    } else if (dstHeaderNotFound && activeSource && !activeDestination) {
      bannerMessage = 'Header group not found for Destination endpoint.';
    } else if (multiAmbiguous) {
      bannerMessage = 'Header group not found — ambiguous match. Awaiting Supervisor verification.';
    ...
    if (srcIsDevice && frozen.dest_is_physical_tb) {
      ...
      bannerMessage = analysisLifecycleChip === 'QUEUED'
        ? 'Automatic TB-location analysis is queued for this panel.'
        : 'Automatic TB-location analysis is processing for this panel.';
    } else if (
      scheduleMismatch
      || panelStatus === 'SCHEDULE_DRAWING_MISMATCH'
    ...
    } else if (analysisFailed && analysisFailureReason) {
      bannerMessage = analysisFailureReason;
    } else if (
      awaitingSupervisor
      || panelStatus === 'TECHNICAL_FAILURE'
      || panelStatus === 'SUPERVISOR_VERIFICATION_REQUIRED'
  ...
  return (
    ...
    >
      <div className="live-tb-modal">
        ...
        {wireChip}
        {!demoMode && analysisLifecycleChip ? (
          <div
            className={`live-tb-analysis-chip live-tb-analysis-chip--${String(analysisLifecycleChip).toLowerCase()}`}
            role="status"
            aria-label={`Analysis state ${analysisLifecycleChip}`}
          >
            Analysis: {analysisLifecycleChip}
            {analysisLifecycleChip === 'FAILED' && analysisFailureReason
