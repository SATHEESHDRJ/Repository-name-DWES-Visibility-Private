...
export default function PauseReasonModal({
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <span className="block text-[11px] font-bold uppercase text-muted mb-1">Your New Assignment</span>
              {isInterchange ? (
                <>
                  <strong className="block whitespace-normal break-words text-[13px] leading-5 text-primary">{selectedTarget.assignment?.panel_name}</strong>
                  <span className="block break-all text-[11px] text-muted">{selectedTarget.assignment?.project_code}</span>
                </>
              ) : (
                <span className="block text-[13px] text-muted italic">None (You will be unassigned)</span>
              )}
            </div>
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
              ...
              <strong className="block whitespace-normal break-words text-[13px] leading-5 text-primary">{me.assignment?.panel_name || 'Your panel'}</strong>
              <span className="block break-all text-[11px] text-muted">{me.assignment?.project_code}</span>
            </div>
          </div>
      ...
      >
        <div className="space-y-3">
          ...
          <div className="max-h-[300px] overflow-y-auto rounded-lg border border-slate-200 bg-white">
            {loadingTargets ? (
              <DwesLoadingCenter label="Loading technicians…" className="p-4 min-h-[6rem]" size="sm" />
            ) : filteredTargets.length === 0 ? (
              <div className="p-4 text-center text-[13px] text-muted">No technicians found.</div>
            ) : (
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <strong className="text-[14px] text-primary">{t.technician_name}</strong>
                          <span className="text-[12px] text-muted">@{t.technician_username}</span>
                        </div>
                        {t.has_assignment ? (
  ...
  return (
    <Modal
      ...
      iconTone="warning"
      size="form"
      bodyClassName="pause-modal-body"
      onClose={onClose}
      closeOnBackdrop={!busy}
      closeOnEscape={!busy}
      footer={(
        <div className="pause-modal-actions">
          <button type="button" className="btn-secondary pause-modal-btn" onClick={onClose} disabled={busy}>
            Cancel
          </button>
          <button
            type="button"
            className={`${isMidChange ? 'btn-primary' : 'btn-warning'} pause-modal-btn`}
            onClick={handleNext}
            disabled={busy || !resolved}
          >
            {isMidChange ? (
              <>Continue to Mid Change →</>
            ) : (
              <>
                <PauseCircle size={18} />
                {busy ? 'Pausing…' : 'Pause'}
    ...
    >
      <div className="pause-modal-content">
        ...
        {reason === 'Other' && (
          <div className="field-with-icon pause-modal-other">
            <span className="field-lead-icon"><MessageCircle size={16} /></span>
            <input
