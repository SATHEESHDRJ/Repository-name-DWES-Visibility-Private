...
const PROJECT_STATUS_META: Record<ProjectRowStatus, { label: string; pill: string }> = {
  completed: { label: 'Completed', pill: 'bg-green-100 text-green-700 border border-green-200' },
  in_progress: { label: 'In Progress', pill: 'bg-yellow-100 text-yellow-800 border border-yellow-200' },
  not_started: { label: 'Not Started', pill: 'bg-slate-100 text-slate-600 border border-slate-200' },
  submitted: { label: 'Submitted', pill: 'bg-indigo-100 text-indigo-800 border border-indigo-200' },
};
...
export default function CompactStatusWorkspace({ isActive = true }: { isActive?: boolean }) {
  ...
  const assignmentByFrame = useMemo(() => {
    ...
    for (const a of assignments) {
      ...
      if (!prev || (a.id ?? 0) > (prev.id ?? 0)) m.set(key, a);
    }
    return m;
  }, [assignments]);
            ...
            <div className="sws-selected-panel" aria-label="Selected panel status">
              <div className="sws-panel-overview">
                <div className="min-w-0">
                  <div className="sws-selected-project-label dw-wim-title-primary">
                    {selectedProject.name}
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="sws-selected-panel-name dw-wim-title-secondary">
                      {compactPanelDisplayName(selectedPanel.panelName)}
                    </h3>
              ...
              <WorkspaceInfoMatrix className="mt-2" aria-label="Selected panel summary">
                <WorkspaceInfoCell label="Technician" emphasis="primary" hideIfEmpty={false}>
                  {assignment?.technician_name || '—'}
                </WorkspaceInfoCell>
                <WorkspaceInfoCell label="Completed" emphasis="default">
                  <span className="tabular-nums">{assignment?.cables_completed ?? 0}</span>
                </WorkspaceInfoCell>
                <WorkspaceInfoCell label="Total" emphasis="default">
                  <span className="tabular-nums">{cablesTotal}</span>
                </WorkspaceInfoCell>
                <WorkspaceInfoCell label="Remaining" emphasis="default">
                  <span className="tabular-nums">{Math.max(0, cablesTotal - (assignment?.cables_completed ?? 0))}</span>
                </WorkspaceInfoCell>
              </WorkspaceInfoMatrix>
