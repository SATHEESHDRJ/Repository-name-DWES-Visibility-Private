...
export default function PanelWorkflowWorkspaceModal({
  ...
  const panelWiringTechLabel = formatPersonLabel(
    activity?.technician ?? activity?.completed_by ?? null,
  );
  ...
  const assignToStage = async () => {
    ...
    if (isWiring && !panelWiringAssigned) {
      ...
      const confirmed = window.confirm('Assign this technician to the panel wiring execution?');
      if (!confirmed) return;
    }
    try {
      ...
      /* error already surfaced via run() */
    }
  };
      ...
      footer={(
        <div className="pw-footer">
          <div className="pw-footer-note">
            ...
            <span>{stageEditDirty ? 'Unsaved stage changes' : 'All changes are saved automatically'}</span>
          </div>
                ...
                <section className="pw-assignments__wiring">
                  ...
                  <div className="pw-assign-status" role="status">
                    <div>
                      ...
                      <strong>{panelWiringAssigned ? `Assigned · ${panelWiringTechLabel}` : 'Not Assigned'}</strong>
                    </div>
                    <div>
                      <span className="pw-assign-status__label">Workflow Wiring Assignment</span>
                      <strong>{wiringWorkflowAssigned ? `Assigned · ${wiringWorkflowTechLabel}` : 'Not Assigned'}</strong>
                    </div>
                    {wiringWorkflowAssigned && !panelWiringAssigned && (
                      <p className="form-error m-0" role="alert">
                        Wiring stage has an assignee, but no panel wiring assignment exists yet.
                        Assign to the Wiring stage again (or use Panel wiring assignment below) to create the Digital Wiring Schedule assignment.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="pw-midchange" role="note">
                    ...
                    <p>
                      Technician reassignment is completed from the active wiring session using Pause → Mid Change.
                      Existing progress and history are preserved.
                    </p>
                  </div>
                </section>
