...
const SLOT_CONFIG: Record<PanelDrawingSlot, {
  ...
  '3d': {
    // Retained slot key for API compatibility; Supervisor UI no longer opens 3D upload.
    label: '3D Model',
  ...
  const upload = async () => {
    ...
    if (existingAsset) {
      const confirmed = await dialog.confirm({
        ...
        message: `This replaces the existing ${cfg.label.toLowerCase()} file for this panel. The previous file stays linked only to this project and panel until archived.`,
        tone: 'warning',
        confirmText: `Replace ${cfg.label}`,
        actionSummary: `Replace “${existingAsset.original_name}” with the newly selected file.`,
        entity: [
          { label: 'Project', value: projectName || projectCode, meta: projectCode, kind: 'project' },
          { label: 'Panel', value: panelName, kind: 'panel' },
          { label: 'Current file', value: existingAsset.original_name, kind: 'file' },
        ],
      });
      if (!confirmed) return;
    }
      ...
      <div className="panel-drawing-upload">
        <ActionStatusPanel
          message={
              ...
              ? `${cfg.label} upload completed for this panel.`
              : error
                ? error
                : uploading
                  ? `Uploading ${file?.name || cfg.label}…`
                  : `Select the ${cfg.label.toLowerCase()} file for this panel, then upload.`
          }
          ...
          actionSummary={
            existingAsset
              ? `Replace existing ${cfg.label} for the selected panel.`
              : `Upload a new ${cfg.label} for the selected panel.`
          }
          entity={[
            { label: 'Project', value: projectName || projectCode, meta: projectCode, kind: 'project' },
            { label: 'Panel', value: panelName, kind: 'panel' },
            ...(file ? [{ label: 'File', value: file.name, meta: formatBytes(file.size), kind: 'file' as const }] : []),
          ]}
        />
