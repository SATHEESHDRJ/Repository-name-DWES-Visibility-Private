      ...
      <div className="ga-engineering-workspace">
        ...
        <div className={`ga-model-canvas${measurementMode ? ' is-measuring' : ''}`}>
          {error ? (
            ...
            <div className="ga-viewer-message" role="status">Loading and validating the engineering model…</div>
          ) : url || sourceModel ? (
            <ModelErrorBoundary key={sourceModel?.uuid || url} onError={() => setError('model-error')}>
              <Canvas shadows dpr={[1, 2]} gl={{ localClippingEnabled: true }} onCreated={({ gl }) => { gl.localClippingEnabled = true; }}>
                ...
                <Suspense fallback={<Html center><span className="ga-model-loading">Loading engineering model…</span></Html>}>
                  <ModelScene
                    url={url}
                    object={sourceModel}
                    format={modelFormat}
