...
import type { TechnicianWorkspaceModule } from '../../components/technician/wiring/wiring-utils';
import type { ActiveWireSnapshot } from '../../types/liveTbView';
import { useTechnicianViewParam } from '../../hooks/useDashboardUrl';
...
export default function TechnicianDashboard() {
  ...
  const [viewParam, setViewParam] = useTechnicianViewParam('panels');
  const tabletMode = viewParam === 'tablet';
  const isFullViewOpen = viewParam === 'full';
  const setTabletMode = useCallback((next: boolean) => {
    setViewParam(next ? 'tablet' : 'panels');
  }, [setViewParam]);
  const setIsFullViewOpen = useCallback((next: boolean) => {
    setViewParam(next ? 'full' : 'panels');
  }, [setViewParam]);
  ...
  const [workspaceModule, setWorkspaceModule] = useState<TechnicianWorkspaceModule>('stripping');
  ...
  const handleSidebarSelect = useCallback((key: string) => {
    ...
    if (!embedWiringSchedule) return;
    if (key === 'tablet') setViewParam('tablet');
    else if (key === 'full') setViewParam('full');
  }, [embedWiringSchedule, setViewParam]);
  ...
  const filterSideNavItems = useMemo(() => {
    ...
    return [
      ...
      // Crimping Report lives only in the workspace module bar — do not repeat in sidebar.
    ];
  }, [
    embedWiringSchedule,
    scheduleFilter,
  ...
  return (
    <DashboardShell
      ...
      heroLive={isLive}
    >
      {!embedWiringSchedule && sectionTitleParts.plain !== 'Panels' && (
        <SectionHeader
      ...
      <section className="dash-module dash-module--wide tech-dash-module">
        {embedWiringSchedule && wiringPanel ? (
          <div className="tech-wiring-shell" aria-live="polite">
            ...
            />
            {/* Progress KPIs + Continue live only in the workstation (progress matrix +
                sidebar Tablet View). Do not repeat them above the wiring schedule. */}
            <WiringTab
              panel={wiringPanel}
              ...
              onWorkspaceModuleChange={setWorkspaceModule}
              onOpenLiveTb={() => {
                setActiveTechnicianFunction(null);
                setLiveTbOpen(true);
              }}
