...
export default function HistoryTab() {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {FILTER_TABS.map(f => (
          ...
          >
            {f.label}
            {f.value !== 'all' && (
              <span className="ml-1 text-[10px] bg-slate-200 text-muted rounded-full px-1.5 py-0.5 font-bold">
      ...
      {loading && <DwesLoadingState label="Loading history…" />}

      {!loading && filtered.length === 0 && (
        <div className="empty-state">
          <DashboardIcon name="history" size={40} />
        ...
        {filtered.map(i => (
          <div key={i.id} className="flex items-center gap-4 p-4 bg-[var(--t-surface-white)] border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-primary truncate">{i.panel_name}</div>
              <div className="text-xs text-muted mt-0.5">{i.project_code} · Technician: {i.technician_name}</div>
              <div className="text-xs text-slate-400 mt-0.5">Inspector: {i.inspector_name} · {new Date(i.created_at).toLocaleString()}</div>
            </div>
            <div className="text-xs text-muted shrink-0">{(i.issues || []).length} issues</div>
            ...
            >
              <DashboardIcon name="explore" size={18} />
            </button>
          </div>
        ))}
      </div>
      ...
      {selected && (
        <Modal title={`Inspection — ${selected.panel_name}`} icon={<ClipboardList />} onClose={() => setSelected(null)} size="lg">
          {detailLoading ? (
            <DwesLoadingState label="Loading…" />
          ) : (
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 tablet-land:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={k} className="bg-slate-50 rounded-xl p-3">
                    ...
                    <div className="text-sm font-medium text-primary mt-0.5">{v}</div>
                  </div>
                ))}
              </div>
              ...
              {/* Inspection checks */}
              <div>
                <div className="text-[11px] font-bold uppercase tracking-wider text-muted mb-2">Inspection Checks</div>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={c.label} className={`rounded-xl p-3 border ${c.val === 'pass' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                      ...
                      {c.note && <div className="text-xs text-muted mt-1">{c.note}</div>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-3">
                <div className="text-[11px] font-bold uppercase tracking-wider text-muted mb-1">Red Markup</div>
                ...
                {selected.redmarkup_note && <div className="text-xs text-muted mt-0.5">{selected.redmarkup_note}</div>}
              </div>
                    ...
                    {selected.issues.map((iss: any) => (
                      <div key={iss.id} className="flex flex-col gap-1 p-3 bg-[var(--t-surface-white)] border border-slate-200 rounded-xl">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-secondary">{iss.description}</span>
                        </div>
                        {iss.location && (
              ...
              {selected.inspection_notes && (
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="text-[11px] font-bold uppercase tracking-wider text-muted mb-1">Notes</div>
                  <div className="text-sm text-secondary">{selected.inspection_notes}</div>
                </div>
              )}
            </div>
          )}
