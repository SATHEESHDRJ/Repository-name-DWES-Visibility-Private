...
function KpiRing({ pct, variant }: { pct: number; variant: 'project' | 'panel' }) {
  const clamped = Math.min(100, Math.max(0, pct));
  return (
    <div
      className={`director-kpi-ring director-kpi-ring--${variant}`}
      style={{ ['--director-kpi' as string]: `${clamped * 3.6}deg` }}
      aria-hidden
    >
      <div>
        <strong>{clamped}%</strong>
        {variant === 'panel' ? <span>DONE</span> : null}
      </div>
    </div>
  );
}
          ...
          <article key={project.projectId} className={`director-project-card${open ? ' is-expanded' : ''}`}>
            <div className="director-project-header">
              ...
              <div className="director-project-summary">
                ...
                <KpiRing pct={project.progressPercentage} variant="project" />
              </div>
            </div>
                      ...
                      project.panels.map(panel => (
                        <article key={panel.panelId} className="director-panel-card">
                          ...
                          <div className="director-panel-performance">
                            <KpiRing pct={panel.progressPercentage} variant="panel" />
