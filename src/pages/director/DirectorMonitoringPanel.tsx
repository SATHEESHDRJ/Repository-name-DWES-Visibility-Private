...
export type DirectorMonitoringPayload = {
  ...
  consolidated_panels: Array<{
    ...
    crimping_required?: number;
    crimping_completed?: number;
    crimping_partial?: number;
    crimping_pending?: number;
    crimping_rework?: number;
    crimping_percent?: number;
    source_stripped?: number;
    source_crimped?: number;
    destination_stripped?: number;
    destination_crimped?: number;
    ready_for_wiring?: number;
    crimping_cut?: number;
    crimping_stripped?: number;
    crimping_crimped?: number;
    crimping_legacy_partial?: number;
    crimping_open_rework?: number;
  }>;
  one_side: {
    summary: {
      src_only: number;
...
export default function DirectorMonitoringPanel({ data, loading, onRefresh }: Props) {
  ...
  const crimpingTotals = useMemo(() => {
    const panels = data?.consolidated_panels || [];
    let required = 0;
    let completed = 0;
    let ready = 0;
    for (const p of panels) {
      required += Number(p.crimping_required) || 0;
      completed += Number(p.crimping_completed) || 0;
      ready += Number(p.ready_for_wiring) || 0;
    }
    const percent = required > 0 ? Math.round((completed / required) * 1000) / 10 : 0;
    return { required, completed, ready, percent, panels: panels.length };
  }, [data]);
  ...
  return (
    <section className="director-monitoring" aria-label="Director monitoring">
      <div className="director-monitoring-kpi-row">
        ...
        <button type="button" className="director-monitoring-kpi" onClick={() => setDrill('consolidated')}>
          <span className="director-monitoring-kpi-label">Stripping / Crimping</span>
          <span className="director-monitoring-kpi-value">{crimpingTotals.percent}%</span>
          <span className="director-monitoring-kpi-meta">
            Complete {crimpingTotals.completed}/{crimpingTotals.required} required · Ready {crimpingTotals.ready}
          </span>
        </button>
              ...
              <tbody>
                {data.consolidated_panels.map(row => (
                  <tr key={row.assignment_id}>
                    ...
                    <td>{row.crimping_required ?? 0}</td>
                    <td>{row.crimping_completed ?? 0}</td>
                    <td>{row.crimping_percent ?? 0}%</td>
                    <td>{row.crimping_rework ?? 0}</td>
                    <td>{row.ready_for_wiring ?? 0}</td>
