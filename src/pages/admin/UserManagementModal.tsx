...
function roleBadgeClass(role: string) {
  ...
  return map[role] ?? 'bg-slate-100 text-muted border-slate-200';
}
...
export function UserManagementModal({ onClose }: { onClose: () => void }) {
  ...
  const handleToggle = async (user: any) => {
    const ok = await dialog.confirm({
      ...
      actionSummary: user.is_active
        ? `Deactivate ${user.full_name} and block new sessions.`
        : `Activate ${user.full_name} and restore login access.`,
      entity: {
        label: 'User',
  ...
  if (loading) {
    return (
      <Modal onClose={onClose} title="User Management" size="lg">
        <DwesLoadingState label="Loading users…" />
        {modals}
      </Modal>
    );
  }
      ...
      <div className="admin-user-table-card">
        <div className="admin-user-table-scroll">
          <table className="admin-user-table">
            <thead>
              <tr>
                <th className="text-[12px] font-bold text-muted uppercase tracking-[0.06em] py-[12px] px-[16px] w-[160px]">Username</th>
                <th className="text-[12px] font-bold text-muted uppercase tracking-[0.06em] py-[12px] px-[16px]">Full Name</th>
                <th className="text-[12px] font-bold text-muted uppercase tracking-[0.06em] py-[12px] px-[16px] w-[180px]">Role</th>
                <th className="text-[12px] font-bold text-muted uppercase tracking-[0.06em] py-[12px] px-[16px] w-[120px]">Status</th>
                <th className="text-[12px] font-bold text-muted uppercase tracking-[0.06em] py-[12px] px-[16px] w-[140px]">Last Login</th>
                <th className="text-[12px] font-bold text-muted uppercase tracking-[0.06em] py-[12px] px-[16px] w-[250px] text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#F1F5F9]">
              {filtered.length === 0 ? (
                ...
                filtered.map((user, idx) => (
                  ...
                  >
                    <td className="px-[16px] py-[8px] text-[14px] font-mono font-medium text-primary truncate">
                      @{user.username}
                    </td>
                    <td className="px-[16px] py-[8px]">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-[12px] font-bold text-muted flex-shrink-0">
                          {initials(user.full_name)}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="text-[14px] font-medium text-primary truncate">{user.full_name}</span>
                          <span className="text-[11px] text-muted truncate">{user.employee_id || 'No ID'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-[16px] py-[8px]">
                    ...
                    <td className="px-[16px] py-[8px]">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wide border
                        ${user.is_active ? 'bg-green-50 text-green-600 border-green-200' : 'bg-slate-100 text-muted border-slate-300'}
                      `}>
                        {user.is_active ? <CheckCircle2 size={12} /> : <UserX size={12} />}
                        {user.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-[16px] py-[8px] text-[13px] text-muted whitespace-nowrap">
      ...
      footer={(
        <div className="flex items-center justify-end gap-3 w-full">
          ...
          <button type="button" className="btn-danger" onClick={() => void onConfirm()} disabled={saving || !confirmed}>
            <Trash2 size={16} />{saving ? 'Deleting…' : 'Delete Permanently'}
          </button>
        </div>
      )}
    >
    ...
    >
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-start gap-4 p-4 rounded-[12px] border border-[#E2E8F0] bg-gradient-to-br from-slate-50 to-[var(--t-surface-white)]">
          <div className="w-14 h-14 rounded-full bg-slate-200 flex items-center justify-center text-[18px] font-bold text-muted shrink-0">
            {initials(user.full_name)}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-[18px] font-bold text-primary truncate" title={user.full_name}>{user.full_name}</div>
        ...
        {mode === 'edit' ? <div className="grid grid-cols-1 tablet-port:grid-cols-2 gap-4">
          ...
          <div className="form-group">
            ...
            <div className="form-input bg-slate-50 text-muted font-mono cursor-default select-none">
              @{user.username}
            </div>
          </div>
          <InputField
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-[10px] border border-[#E2E8F0] bg-[var(--t-surface-white)]">
      ...
      <div className="min-w-0">
        ...
        <div className={`text-[13px] font-medium text-primary truncate ${mono ? 'font-mono' : ''}`} title={value}>{value}</div>
      </div>
    </div>
  );
}
    ...
    >
      {!done ? (
        ...
        <div className="flex flex-col items-center gap-3 py-6 text-center">
          ...
          <div className="text-[16px] font-bold text-primary">Password reset</div>
          <div className="text-[13px] text-muted">{user.full_name} can now log in with the new password.</div>
        </div>
      )}
    </Modal>
  );
