...
export type PanelAutoExtractConfidence = 'CONFIRMED' | 'HIGH_CONFIDENCE' | 'REVIEW_REQUIRED' | 'UNRESOLVED';

export interface PanelAutoExtractSourcedField<T = string | number | boolean | null> {
  value: T;
  confidence: PanelAutoExtractConfidence;
  source: string;
  source_page?: string | null;
  source_entity?: string | null;
}

export interface PanelAutoExtractResponse {
  extract: {
    scanned_sources: Array<{ drawing_id: string; original_name: string; role: string; pages_scanned: number; text_quality: number }>;
    views_detected: string[];
    fields: Record<string, PanelAutoExtractSourcedField>;
    components: Array<PanelAutoExtractSourcedField<string> & { type: string; label: string }>;
    schedule_match: {
      matched_tags: string[];
    ...
    form: {
      ...
      components: Array<{ label: string; type: string; confidence: PanelAutoExtractConfidence; source: string }>;
    };
    notes: string[];
    auto_fix: {
